...
<div class="p-6 space-y-6 animate-fade-in">
    {{-- Header Section --}}
    <div class="mb-6">
        <h2 class="text-2xl font-black text-gray-800 tracking-tight uppercase">Update Data Kelas</h2>
        <p class="text-xs font-bold text-gray-400 mt-1 uppercase tracking-widest">Perbarui informasi kelas dan penetapan wali kelas</p>
    </div>

    {{-- Form Container --}}
    <div class="max-w-4xl bg-white rounded-2xl shadow-sm border border-gray-100 overflow-hidden border-t-4 border-t-blue-600">
        <form action="{{ route('admin.kelas.update', $kela->id) }}" method="POST" class="p-8 space-y-6">
            @csrf 
            @method('PUT')
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                {{-- Pilih Wali Kelas (Konsisten dengan Create) --}}
                <div class="space-y-2">
            ...
            {{-- Info Tip --}}
            <div class="p-4 bg-amber-50 rounded-xl border border-amber-100 flex items-center gap-3">
                <div class="w-8 h-8 rounded-full bg-amber-500 flex items-center justify-center flex-shrink-0 shadow-sm">
                    <i class="fa-solid fa-pen-to-square text-white text-xs"></i>
                </div>
                <p class="text-[11px] text-amber-700 font-bold leading-tight uppercase tracking-wider">
                    Perubahan pada data ini akan langsung memperbarui status perwalian di laporan akademik.
                </p>
            </div>
            ...
            <div class="pt-6 flex justify-between items-center border-t border-gray-50 mt-4">
                <a href="{{ route('admin.kelas.index') }}" 
                   class="px-8 py-2.5 rounded-lg text-xs font-black text-white bg-rose-600 hover:bg-rose-700 transition-all shadow-md shadow-rose-100 uppercase tracking-widest text-center">
                    Batal
                </a>
