...
<div class="p-4 space-y-4 animate-fade-in">
    ...
    <div class="flex flex-col md:flex-row md:items-center justify-between gap-3">
        ...
        {{-- Tombol Cetak Bulanan (Hanya muncul jika mode bulanan dan kelas dipilih) --}}
            ...
            <form method="GET" action="{{ route('admin.absensi.index') }}">
                <div class="flex flex-wrap items-end gap-3">
                    <div class="flex flex-col gap-1.5">
                        ...
                        <select name="tahun_ajaran" class="h-9 w-32 bg-white border border-gray-200 text-gray-700 text-[10px] rounded-lg px-3 font-bold outline-none focus:border-blue-500 cursor-pointer">
                    ...
                    <div class="flex flex-col gap-1.5">
                        ...
                        <select name="semester" class="h-9 w-24 bg-white border border-gray-200 text-gray-700 text-[10px] rounded-lg px-3 font-bold outline-none focus:border-blue-500 cursor-pointer">
                    ...
                    <div class="flex flex-col gap-1.5">
                        ...
                        <select name="mode" onchange="this.form.submit()" class="h-9 bg-white border border-gray-200 text-gray-700 text-[10px] rounded-lg px-3 font-bold outline-none focus:border-blue-500 cursor-pointer">
                    ...
                    <div class="flex flex-col gap-1.5">
                        ...
                        @if(request('mode') == 'bulanan')
                            <select name="filter_month" class="h-9 w-32 bg-white border border-gray-200 text-gray-700 text-[10px] rounded-lg px-3 font-bold outline-none focus:border-blue-500 cursor-pointer">
                        ...
                        @else
                            <input type="date" name="filter_date" value="{{ request('filter_date', date('Y-m-d')) }}" 
                                   class="h-9 bg-white border border-gray-200 text-gray-700 text-[10px] rounded-lg px-3 font-bold outline-none focus:border-blue-500 cursor-pointer">
                        @endif
                    </div>

                    <div class="flex flex-col gap-1.5">
                        <label class="text-[8px] font-black text-blue-600 uppercase tracking-widest ml-1">Kelas</label>
                        <select name="kelas" class="h-9 w-32 bg-white border border-gray-200 text-gray-700 text-[10px] rounded-lg px-3 font-bold outline-none focus:border-blue-500 cursor-pointer">
                    ...
                    <button type="submit" class="h-9 bg-blue-600 hover:bg-blue-700 text-white font-black px-5 rounded-lg shadow-sm transition-all active:scale-95 uppercase text-[9px] tracking-widest flex items-center gap-2">
            ...
            <table class="w-full text-left border-collapse">
                <thead>
                    <tr class="bg-gray-50/50">
                        ...
                        <th class="px-4 py-3 text-center text-[9px] font-black text-gray-400 uppercase tracking-widest border-b border-gray-50">Mapel</th>
                ...
                <tbody class="divide-y divide-gray-50">
                    ...
                    <tr class="hover:bg-blue-50/10 transition-all group">
                        <td class="px-6 py-3 text-gray-300 font-bold text-center text-[10px]">{{ $loop->iteration }}</td>
                        <td class="px-4 py-3">
                            <div class="flex flex-col">
                                <span class="font-black text-slate-700 uppercase text-xs">{{ $a->siswa->nama ?? 'Unknown' }}</span>
                                <span class="text-[8px] text-gray-400 font-bold uppercase tracking-tighter">NISN: {{ $a->siswa->nisn ?? '-' }}</span>
                            </div>
                        </td>
                        ...
                        <td class="px-4 py-3 text-center">
                            <span class="text-slate-600 text-[9px] font-bold uppercase tracking-wide">
                        ...
                        <td class="px-6 py-3 text-right">
                            <div class="flex items-center justify-end gap-2">
                                <a href="{{ route('admin.absensi.edit', $a->id) }}" class="inline-flex items-center justify-center w-7 h-7 bg-slate-50 text-slate-400 rounded-lg border hover:bg-blue-600 hover:text-white transition-all shadow-sm">
                                ...
                                <form action="{{ route('admin.absensi.destroy', $a->id) }}" method="POST" onsubmit="return confirm('Hapus data absensi ini?')">
                                    @csrf @method('DELETE')
                                    <button type="submit" class="inline-flex items-center justify-center w-7 h-7 bg-slate-50 text-rose-400 rounded-lg border hover:bg-rose-600 hover:text-white transition-all shadow-sm">
                    ...
                    @empty
                    <tr>
                        <td colspan="7" class="px-6 py-16 text-center text-gray-400 uppercase text-[9px] font-black tracking-widest">Data Tidak Ditemukan</td>
                    </tr>
                    @endforelse
                </tbody>
            </table>
