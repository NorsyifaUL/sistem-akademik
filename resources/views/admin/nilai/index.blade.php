...
<div class="p-6 space-y-6 animate-fade-in">
    ...
    <div class="flex flex-col md:flex-row md:items-center justify-between gap-4">
        ...
        {{-- Tombol Cetak Legger telah dihilangkan dari sini --}}
    </div>
            ...
            <form action="{{ route('admin.nilai.index') }}" method="GET" class="flex flex-col md:flex-row items-end gap-4">
                <div class="w-full md:w-1/3 space-y-2">
                    ...
                    <div class="relative group">
                        ...
                        <select name="kelas" class="w-full pl-12 pr-10 py-4 rounded-2xl border-2 border-gray-100 bg-white focus:border-blue-600 focus:ring-0 text-xs font-bold transition-all outline-none appearance-none cursor-pointer">
                            <option value="" disabled {{ !$kelasTerpilih ? 'selected' : '' }}>-- Semua Kelas --</option>
                            @foreach($data_kelas as $k)
                                <option value="{{ $k }}" {{ $kelasTerpilih == $k ? 'selected' : '' }}>
                                    Kelas {{ $k }}
                                </option>
                            @endforeach
                        </select>
            ...
            <table class="w-full text-left border-collapse">
                ...
                <tbody class="divide-y divide-gray-50">
                    ...
                    <tr class="group hover:bg-blue-50/30 transition-all duration-300">
                        ...
                        <td class="px-6 py-6 text-center">
                            <div class="text-sm font-black {{ ($s->rata_rata_akhir ?? 0) < 75 ? 'text-rose-500' : 'text-blue-600' }}">
                                {{ number_format($s->rata_rata_akhir ?? 0, 2) }}
                            </div>
                        </td>
                        <td class="px-8 py-6">
                    ...
                    <tr>
                        <td colspan="5" class="px-8 py-20 text-center">
                            ...
                            <h4 class="text-sm font-black text-gray-400 uppercase tracking-widest">Tidak ada data siswa di kelas ini</h4>
                        </td>
                    </tr>
                    @endforelse
                </tbody>
