...
<div class="p-8 space-y-6 animate-fade-in bg-gray-50/50 min-h-screen">
    ...
    {{-- HEADER SECTION (Sekarang Lebih Bersih) --}}
    <div class="flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
            ...
            <p class="text-sm text-slate-500 mt-2">Daftar rekaman notifikasi sistem yang tersimpan di database.</p>
        </div>
    ...
    <div class="bg-white rounded-[2rem] border border-gray-200 shadow-sm overflow-hidden border-t-4 border-t-indigo-500">
            ...
            <form action="{{ route('admin.notifikasi.index') }}" method="GET" class="flex items-center gap-3 w-full md:w-auto">
                <div class="relative w-full md:w-64 group">
                    <select name="kelas" onchange="this.form.submit()" 
                        class="appearance-none w-full bg-white border border-gray-200 text-slate-700 text-sm rounded-xl focus:ring-2 focus:ring-indigo-500 block pl-4 pr-10 py-2.5 transition-all outline-none shadow-sm">
                ...
                @if(request('kelas'))
                    <a href="{{ route('admin.notifikasi.index') }}" class="text-[10px] font-bold text-red-500 hover:bg-red-50 px-3 py-2 rounded-lg transition-all uppercase tracking-widest">
                        Reset
                    </a>
                @endif
            </form>

            <span class="text-[10px] font-bold text-slate-400 uppercase tracking-[0.2em]">Database Records</span>
        </div>
            ...
            <table class="w-full text-left border-collapse">
                <thead>
                    <tr class="text-[11px] font-bold text-slate-400 uppercase tracking-widest border-b border-gray-100">
                        ...
                        <th class="px-8 py-5 text-indigo-600/70">Isi Notifikasi</th>
                        <th class="px-8 py-5 text-center text-indigo-600/70">Aksi</th>
                    </tr>
                </thead>
                <tbody class="divide-y divide-gray-50">
                    @forelse($notifikasis as $n)
                        <tr class="group hover:bg-indigo-50/10 transition-all">
                            ...
                            <td class="px-8 py-6">
                                <div class="max-w-md bg-slate-50 border border-slate-100 p-3 rounded-xl group-hover:bg-white group-hover:border-indigo-100 transition-all italic text-slate-600 text-sm">
                                    "{{ $n->isi_pesan }}"
                                </div>
                            </td>
                            <td class="px-8 py-6 text-center">
                                <form action="{{ route('admin.notifikasi.destroy', $n->id) }}" method="POST" onsubmit="return confirm('Hapus log ini?')">
                                    @csrf @method('DELETE')
                                    <button class="h-9 w-9 inline-flex items-center justify-center rounded-xl text-slate-300 hover:text-red-600 hover:bg-red-50 transition-all">
                    ...
                    @empty
                        <tr>
                            <td colspan="4" class="px-6 py-24 text-center text-slate-300 uppercase text-xs font-bold tracking-widest">Data tidak ditemukan</td>
                        </tr>
                    @endforelse
                </tbody>
            </table>
        ...
        {{-- FOOTER CARD: TOTAL LOG & PAGINATION --}}
        <div class="px-8 py-6 bg-slate-50/50 border-t border-gray-100 flex flex-col md:flex-row justify-between items-center gap-4">
            {{-- Statistik di Kiri Bawah --}}
            <div class="flex items-center gap-3">
                <div class="h-8 w-1 bg-indigo-500 rounded-full"></div>
                <div>
                    <p class="text-[10px] font-bold text-slate-400 uppercase tracking-widest leading-none">Total Keseluruhan</p>
                    <p class="text-sm font-black text-slate-800 tracking-tight">{{ $notifikasis->total() }} <span class="font-medium text-slate-500 text-[11px]">Entri Log Terarsip</span></p>
                </div>
            </div>
