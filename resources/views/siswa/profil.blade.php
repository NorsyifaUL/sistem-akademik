...
<div class="max-w-4xl text-left animate-fade-in p-6">
    ...
    @if(session('success'))
        <div class="mb-6 p-4 bg-blue-50 border-l-4 border-blue-500 text-blue-700 rounded-r-xl shadow-sm text-xs font-bold flex items-center gap-2">
    ...
    <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
        ...
        <div class="md:col-span-1 space-y-6">
            <div class="bg-white rounded-3xl border border-gray-100 shadow-sm p-6 text-center">
                <div class="w-24 h-24 bg-blue-100 rounded-full mx-auto mb-4 flex items-center justify-center border-4 border-blue-50 text-blue-600">
                    <i class="fa-solid fa-user-graduate text-4xl"></i>
                </div>
                <h3 class="text-sm font-black text-gray-800 capitalize">{{ Auth::user()->name }}</h3>
                <p class="text-[10px] font-bold text-blue-600 uppercase tracking-tighter mb-4">Siswa Aktif Smanja</p>
                
                <div class="pt-4 border-t border-gray-50 flex flex-col gap-3">
                    <div class="flex justify-between text-[10px] font-bold">
                        <span class="text-gray-400 uppercase">NISN</span>
                        <span class="text-gray-700">{{ Auth::user()->siswa->nisn ?? '-' }}</span>
                    </div>
                    <div class="flex justify-between text-[10px] font-bold">
                        <span class="text-gray-400 uppercase">Kelas</span>
                        <span class="text-blue-600">Kelas {{ Auth::user()->siswa->kelas->nama_kelas ?? 'Belum Diplot' }}</span>
                    </div>
                </div>
            </div>
        </div>
        ...
        <div class="md:col-span-2 space-y-6">
            <div class="bg-white rounded-3xl border border-gray-100 shadow-sm overflow-hidden border-t-4 border-t-blue-600">
                ...
                <form action="{{ route('siswa.profil.update') }}" method="POST" class="p-6 space-y-4">
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                        ...
                        <div class="space-y-1.5">
                            <label class="block text-[10px] font-bold text-gray-400 uppercase tracking-wider">WA Orang Tua (Gateway)</label>
                            <input type="text" value="{{ Auth::user()->siswa->no_wa_ortu ?? '-' }}" disabled
                                   class="w-full bg-gray-100 border-gray-100 rounded-xl px-4 py-2 text-xs font-bold text-gray-400 cursor-not-allowed">
                        </div>
                    </div>
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                        <div class="space-y-1.5">
                            ...
                            <div class="relative">
                                <input type="password" name="password" id="pass_siswa"
                                       class="w-full bg-gray-50 border-gray-200 rounded-xl px-4 py-2 text-xs font-bold text-gray-700 focus:ring-2 focus:ring-blue-500 focus:border-blue-500 transition-all shadow-inner"
                                       placeholder="Kosongkan jika tidak ganti">
                                <button type="button" onclick="togglePass('pass_siswa', 'eye-siswa-1')" class="absolute inset-y-0 right-0 pr-3 flex items-center text-gray-400 hover:text-blue-600 transition-colors">
                        ...
                        <div class="space-y-1.5">
                            ...
                            <div class="relative">
                                ...
                                <button type="button" onclick="togglePass('conf_siswa', 'eye-siswa-2')" class="absolute inset-y-0 right-0 pr-3 flex items-center text-gray-400 hover:text-blue-600 transition-colors">
                    ...
                    <div class="pt-4 flex items-center justify-between gap-4">
                        ...
                        <button type="submit" class="bg-blue-600 hover:bg-blue-700 text-white px-8 py-2.5 rounded-xl font-black text-[10px] uppercase tracking-widest shadow-lg shadow-blue-100 transition-all flex items-center gap-2">
